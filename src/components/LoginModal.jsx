...
export default function LoginModal({ onClose, onLogin, user, onLogout }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.7)', backdropFilter:'blur(8px)' }} onClick={onClose}>
      <div style={{ background:THEME.bgMainF2, border:`1px solid ${THEME.borderSubtle}`, borderRadius:16, padding:'36px 32px', width:400, backdropFilter:'blur(20px)', boxShadow:`0 0 60px ${THEME.celeste12}`, borderTop:`1px solid ${THEME.celeste30}` }} onClick={e=>e.stopPropagation()}>
        ...
        {!user ? (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <button onClick={handleLogin} style={{
              width:'100%', padding:'22px', marginTop:8,
              background: logging ? THEME.celeste12 : THEME.gold10,
              border: `1px solid ${logging ? THEME.celeste35 : THEME.gold40}`,
              borderRadius:14, color: logging ? THEME.celeste : THEME.gold,
              fontFamily:"'Space Grotesk',sans-serif", fontSize:'1rem', letterSpacing:'0.25em', fontWeight:700,
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <button onClick={onLogout} style={{ width:'100%', padding:'20px', background:'rgba(255,80,60,0.06)', border:'1px solid rgba(255,80,60,0.18)', borderRadius:14, color:'rgba(255,80,60,0.6)', fontFamily:"'Space Grotesk',sans-serif", fontSize:'0.95rem', letterSpacing:'0.25em', fontWeight:700, cursor:'pointer', textTransform:'uppercase', transition:'all 0.2s' }}
